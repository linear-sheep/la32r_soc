// Module: dvi_extfb_reader (DVI VDMA)
// Features: AXI Burst, 1024-depth FIFO (1 BRAM), 1-cycle latency

module dvi_extfb_reader #(
    parameter [31:0] FB_BASE = 32'h1c40_0000
)(
    input  wire        aclk,
    input  wire        aresetn,
    ...
    assign m_rready  = (state == ST_READ); // Keep ready during burst mapped
    ...
    always @(posedge aclk) begin
        if (!aresetn) begin
            ...
            if (vsync_rise) begin
                state <= ST_IDLE;
                fetch_addr <= fb_en ? fb_base : FB_BASE;
            end else begin
                case (state)
                    ST_IDLE: begin
    ...
    wire [7:0] px8 = (pixel_mux_d == 2'b00) ? raw_data[7:0] :
                     ...
                     (pixel_mux_d == 2'b10) ? raw_data[23:16] :
                                              raw_data[31:24];
